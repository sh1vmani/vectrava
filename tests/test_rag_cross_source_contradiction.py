"""Tests for the cross_source_contradiction rag probe.
...
recover them from the request body's messages[1] (the user message carrying the
sources block) with a [0-9a-f]{16} match and echo them back.
"""
...
from vectrava.rag.probes.cross_source_contradiction import (
    INJECTION_PROMPTS,
    CrossSourceContradictionProbe,
)
...
def test_canary_in_response_emits_finding() -> None:
    ...
    with _client(handler) as client:
        findings = CrossSourceContradictionProbe().run(_ctx(client))

    assert len(findings) == len(INJECTION_PROMPTS)
    for finding in findings:
        assert finding.level == Severity.HIGH
        assert finding.rule_id == "cross_source_contradiction"
        assert finding.probe == "rag.cross_source_contradiction"
...
def test_canary_is_fresh_per_run() -> None:
    ...
    with _client(handler) as client:
        CrossSourceContradictionProbe().run(_ctx(client))
        CrossSourceContradictionProbe().run(_ctx(client))
...
def test_missing_credentials_raises_probe_error() -> None:
    ...
    with (
        ...
        CrossSourceContradictionProbe().run(_ctx(client, credentials=None))
...
def test_required_classvars_set() -> None:
    assert CrossSourceContradictionProbe.name == "cross_source_contradiction"
    assert isinstance(CrossSourceContradictionProbe.name, str)
    assert CrossSourceContradictionProbe.module == "rag"
    assert isinstance(CrossSourceContradictionProbe.module, str)
    assert isinstance(CrossSourceContradictionProbe.description, str)
    assert CrossSourceContradictionProbe.baseline_severity == Severity.HIGH
    assert isinstance(CrossSourceContradictionProbe.baseline_severity, Severity)
    assert CrossSourceContradictionProbe.estimated_tokens_per_run == 3 * 500
    assert isinstance(CrossSourceContradictionProbe.estimated_tokens_per_run, int)
...
def test_num_sources_pads_with_filler_when_greater_than_three() -> None:
    ...
    with _client(handler) as client:
        ...
        CrossSourceContradictionProbe().run(ctx)
