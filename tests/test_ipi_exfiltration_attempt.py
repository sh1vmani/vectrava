"""Tests for the exfiltration_attempt ipi probe.
...
match and echo it back to simulate a leak.
"""
...
from vectrava.ipi.probes.exfiltration_attempt import (
    INJECTION_PROMPTS,
    ExfiltrationAttemptProbe,
)
...
def test_canary_in_response_emits_finding() -> None:
    ...
    with _client(handler) as client:
        findings = ExfiltrationAttemptProbe().run(_ctx(client))

    assert len(findings) == len(INJECTION_PROMPTS)
    for finding in findings:
        assert finding.level == Severity.CRITICAL
        assert finding.rule_id == "exfiltration_attempt"
        assert finding.probe == "ipi.exfiltration_attempt"
...
def test_canary_is_fresh_per_run() -> None:
    ...
    with _client(handler) as client:
        ExfiltrationAttemptProbe().run(_ctx(client))
        ExfiltrationAttemptProbe().run(_ctx(client))
...
def test_missing_credentials_raises_probe_error() -> None:
    ...
    with (
        ...
        ExfiltrationAttemptProbe().run(_ctx(client, credentials=None))
...
def test_required_classvars_set() -> None:
    assert ExfiltrationAttemptProbe.name == "exfiltration_attempt"
    assert isinstance(ExfiltrationAttemptProbe.name, str)
    assert ExfiltrationAttemptProbe.module == "ipi"
    assert isinstance(ExfiltrationAttemptProbe.module, str)
    assert isinstance(ExfiltrationAttemptProbe.description, str)
    assert ExfiltrationAttemptProbe.baseline_severity == Severity.CRITICAL
    assert isinstance(ExfiltrationAttemptProbe.baseline_severity, Severity)
    assert ExfiltrationAttemptProbe.estimated_tokens_per_run == 3 * 400
    assert isinstance(ExfiltrationAttemptProbe.estimated_tokens_per_run, int)
