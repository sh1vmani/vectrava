...
3. concurrency_amplification and rate_limit_bypass reported saw_enforcement false, which read as a tested negative when the probe had only observed no throttling across a fixed burst. Investigation found the no-limit-vs-bypassed distinction is infeasible by burst design: an absent limit and a limit not triggered at the burst volume both serve every request, and the only signal that could separate them is optional provider-specific headers most targets do not send. Shipped: renamed the evidence key to enforcement_observed and reworded the messages to state only the bounded observation, no fabricated distinction (this commit).
4. The HTML report banner keys on execution success, not findings, so a report containing a critical finding shows a green completed-successfully banner. Fix path: make the banner findings-aware. Shipped: findings-aware HTML banner (this commit).
5. vtra scope new-key was observed writing the private key world-readable (mode 644) on Windows. Investigation found the command already chmods 0o600, but on Windows Path.chmod toggles only the read-only bit and never raises, so the chmod silently under-delivers and the operator gets no signal. Shipped: a Windows-only stderr warning to protect the key directory, since file-mode enforcement is unavailable there; the chmod is correct on POSIX and unchanged (this commit).
6. Every probe sets requires_credentials, so the no-auth Ollama path still requires a credential flag. Recon during follow-up found the README Ollama quickstart already covers this: it exports a throwaway OLLAMA_KEY and passes --api-key-env, with a note explaining the BYOK gate requires a credential that Ollama ignores. Decoupling requires_credentials from the target was considered and judged disproportionate (it touches all 18 probes and the gate, and the uniform BYOK path is defensible). Already satisfied by documentation; no change required.
7. The model preference list names llama3.2:1b, but a 3b pull was the path of least resistance early in the pass. Recon during follow-up found this described operator behavior during validation, not a doc defect: the README quickstart already pulls llama3.2:1b and recommends it, matching _OLLAMA_PREFERRED_MODELS exactly, with no drift between doc and code. Already satisfied; no change required.
